...
function [iweights, rate] = ggp_sample_weights(iweights, iC, iM, epsilon, alpha, sigma, tau, phi, gvec, settings)
...
for t=1:T
    ...
    grad1 = grad_U(t, m, w, w_rem, counts, sigma, tau, phi, g);
    ...
    for lp=1:settings.leapfrog.L
        ...
        if lp~=settings.leapfrog.L            
            pprop = pprop  - eps.* grad_U(t, m, exp(logwprop), w_rem, counts, sigma,  tau, phi, g);
        end
    end
    wprop = exp(logwprop);    
    pprop = pprop - (eps/2).*grad_U(t, m, wprop, w_rem, counts, sigma, tau, phi, g);
...
switch t
    case 1
        %         temp1 = - g*(sumall_wprop^2) + g*(sumall_w^2) ...
        %             + sum((m +max( counts(:, t)-sigma,0) -1).*(logwprop - logw) )...
        %             - (tau+ phi)*(sum_wprop - sum_w);
        temp1 = - g*(sumall_wprop^2) + g*(sumall_w^2) ...
            + sum((m + counts(:, t)-sigma -1).*(logwprop - logw) )...
            - (tau+ phi)*(sum_wprop - sum_w);
    ...
    otherwise
        %                 temp1 = - g*(sumall_wprop^2) + g*(sumall_w^2) ...
        %                     + sum((m +max(counts(:, t) +counts(:, t-1) - sigma,0) -1).*(logwprop - logw) )...
        %                     - (tau+ 2*phi)*(sum_wprop - sum_w);
        temp1 = - g*(sumall_wprop^2) + g*(sumall_w^2) ...
            + sum((m + counts(:, t) +counts(:, t-1) - sigma -1).*(logwprop - logw) )...
            - (tau+ 2*phi)*(sum_wprop - sum_w);
...
%% Gradient
function [out] = grad_U(t, mm, wvec, w_rem, counts, sigma, tau, phi, gv)
%     N = size(M,2);
...
% else
    switch t
        case 1
            ...
            out= -(mm + counts( :,  t)-sigma) +wvec.*(2*gv*sum(wvec)+2*gv*w_rem +tau + phi);
       % case N
        ...
        otherwise
            ...
            out = - (mm + counts(:, t-1) + counts(:, t)-sigma )+ wvec.*temp;
    end
