function [weights C] = ggp_sample_weights_add(C, weights, M, alpha, sigma, tau, phi, gvec)
[K T] = size(weights);
...
for t=T-1:-1:1
    ...
    ind = find(sum(M(:, t+1:end), 2) ==0); %nodes that stop having links after t 
    ...
    for p = t:T-1
        wprop(:, it) = (cprop(:, it)>0).*gamrnd(max(cprop(:, it)-sigma, 0), 1/(phi+tau));
