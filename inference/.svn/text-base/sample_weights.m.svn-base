...
sumall_w = sum_w + w_rem;
    ...
    grad1 = grad_U(t, M, w, w_rem, counts, alpha, tau, phi, settings);
    ...
    for lp=1:settings.leapfrog.L
        ...
        if lp~=settings.leapfrog.L            
            pprop = pprop  - eps.* grad_U(t, M, exp(logwprop), w_rem, counts, alpha, tau, phi, settings);;
        end
    end
    wprop = exp(logwprop);    
    pprop = pprop - (eps/2).*grad_U(t, M, wprop, w_rem, counts, alpha, tau, phi, settings);
...
if N ==1
    ...
    switch t
        case 1
            ...
            out= -(M(:, 1) + counts( :,  1)) +wvec.*(2*sum(wvec)+2*w_rem +tau + phi(1, :));
        case N
